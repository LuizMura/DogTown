...
if (iconeCarrinho) {
  iconeCarrinho.addEventListener("click", (e) => {
    if (carrinhoAside) {
      // Na loja, o clique abre/fecha o resumo em vez de navegar.
      e.preventDefault();
      e.stopPropagation();
...
document.addEventListener("click", (e) => {
  ...
  if (
    carrinhoAside &&
    iconeCarrinho &&
    !carrinhoAside.contains(e.target) &&
...
function atualizarCarrinho() {
  ...
  if (contadorEl) {
    ...
    contadorEl.classList.toggle("has-items", totalItens > 0);
  }
...
function adicionarAoCarrinho(nome, preco, volume) {
  const itemExistente = carrinho.find(
    (item) => item.nome === nome && item.volume === volume,
  );
  if (itemExistente) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  enviarPedido?.addEventListener("click", () => {
    ...
    if (!termos) {
      ...
      alert(
        "Você precisa declarar ter mais de 18 anos antes de enviar o pedido.",
      );
      return;
    } else {
